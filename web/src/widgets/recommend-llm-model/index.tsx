...
type RecommendOutput = NonNullable<ReturnType<typeof useToolInfo<"recommend-llm-model">>["output"]>;
type Recommendation = RecommendOutput["recommendations"][number];

function RecommendModel() {
  const { output } = useToolInfo<"recommend-llm-model">();
  ...
  const {
    ...
    candidateCount, rankedCount, roiCalculatorUrl, source, eloAsOf, dataAsOf, error,
  } = output;
...
function PodiumCard({ rec: r }: { rec: Recommendation }) {
  return (
    <Card accent={r.rank === 1}>
      ...
      <div style={{ fontSize: "18px", fontWeight: 700 }}>{formatBudget(r.monthlyBudget)}<span style={{ fontSize: "11px", fontWeight: 400, color: "var(--text-muted)" }}>/mo</span></div>
